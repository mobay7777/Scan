...
HomeController.get('/circulatingSupply', async (req, res) => {
    ...
    let foundationBalance = await web3.eth.getBalance(contractAddress.RupxFoundation)
    let teamBalance = await web3.eth.getBalance(contractAddress.RupxTeam)
...
HomeController.get('/totalcoins', async (req, res) => {
    ...
    return res.json(circulatingNumber.toNumber())
})
